...
async function sendClassReminderEmail(toEmail, details) {
    ...
    try {
        const { courseName, time, partnerName, role } = details;
        const subject = `⏰ แจ้งเตือน: วันนี้คุณมีนัดหมายวิชา "${courseName}"`;

        const body = `
            <p>สวัสดีคุณ <strong>${role === 'student' ? 'นักเรียน' : 'ติวเตอร์'}</strong>,</p>
            <p>อย่าลืมนะคะ! วันนี้คุณมีนัดหมายการติว/สอน รายละเอียดดังนี้:</p>
            ...
            <p>ขอให้วันนี้เป็นวันที่ดีของการเรียนรู้นะคะ!</p>
            <br>
            <center>
        ...
        await transporter.sendMail({
            ...
            to: toEmail,
            subject: subject,
            html: wrapHtml('แจ้งเตือนตารางเรียนวันนี้', body)
        });
